...
def clean_tweet(tweet):
    ...
    tweet = re.sub(
        r"(@\[A-Za-z0-9]+)|([^0-9A-Za-z \t])|(\w+:\/\/\S+)|^rt|http.+?", "", tweet)
    return tweet
...
def first_pronouns_metric(tweet):
    tweet_clean = clean_tweet(tweet)
    first_person_pronoun_dict = ["i", "me",
...
def get_tweet_vector(tweet):
    ...
    pronouns = [first_pronouns_metric(tweet)]*weight_pronouns_metric + [
        second_third_pronouns_metric(tweet)]*weight_pronouns_metric
    textblob = [TextBlob_metrics(tweet)[0]]*weight_textblob + \
        [TextBlob_metrics(tweet)[1]]*weight_textblob
    anew = [anew_metric(tweet)[0]]*weight_anew_metric + [anew_metric(tweet)[1]] * \
        weight_anew_metric + [anew_metric(tweet)[2]]*weight_anew_metric
    pos = [pronominalisation_index(tweet)]*weight_pos_metric + [readiness_to_action_index(
        tweet)]*weight_pos_metric + [punctuation_metric(tweet)]*weight_pos_metric
